...
export const ItemsInput = ({ value, onValueChange, ...props }) => {
  const [inputValue, setInputValue] = useState("");

  const openScan = useScan({ onData: handleScan, allowedModels: ["items"] });
  ...
  function handleScan(ids) {
    if (!Array.isArray(ids)) {
      ids = [ids];
    }
    onValueChange?.([...new Set([...value, ...ids])]);
  }
    ...
    <Fragment>
      <div className="flex items-center gap-x-3">
        <Input
          ...
          {...props}
        />
        <IconButton icon="qrcode" type="button" disabled={props.disabled} onClick={openScan} />
      </div>
